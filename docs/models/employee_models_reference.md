...
**IMPORTANTE**: Il modulo Employee NON ha un modello `TimeEntry`. Utilizzare sempre `WorkHour`.
...
❌ **TimeEntry** - NON ESISTE, usare `WorkHour`
...
### PRIMA (❌ ERRATO)
```php
use Modules\Employee\Models\TimeEntry;          // ❌ NON ESISTE
use Modules\Employee\Enums\TimeEntryTypeEnum;   // ❌ NON ESISTE  
use Modules\Employee\Enums\TimeEntryStatusEnum; // ❌ NON ESISTE

$entries = TimeEntry::where('employee_id', $userId)->get(); // ❌ ERRORE
```
...
**RICORDA**: SEMPRE usare `WorkHour`, MAI `TimeEntry` che non esiste.
